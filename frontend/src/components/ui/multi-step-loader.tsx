      ...
      {loadingStates.map((loadingState, index) => {
        ...
        return (
          ...
          >
            <div>
              {index > value && (
                <CheckIcon className="text-white" /> // Change to white
              )}
              {index <= value && (
                <CheckFilled
                  className={cn(
                    "text-white", // Change to white
  ...
  return (
    <AnimatePresence mode="wait">
      {loading && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 0.7 }} // Adjust opacity for transparency
          exit={{ opacity: 0 }}
          className="w-full h-full fixed inset-0 z-[100] flex items-center justify-center backdrop-blur-2xl"
          style={{ backgroundColor: 'rgba(0, 0, 0, 0.7)' }} // Background color with transparency
        >
          <div className="h-96 relative text-blue">
